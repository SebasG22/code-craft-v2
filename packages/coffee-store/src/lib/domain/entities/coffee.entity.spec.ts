...
describe('Coffee Entity', ()=> {
    ...
    const ingredients = [{name: 'milk foam', quantity: 20},{name: "espresso", quantity: 30 } ];
    
    const coffeIngredientes = ingredients.map((ingredient)=> CoffeeIngredient.create(ingredient));
    const capucino = CoffeeEntity.create({
        ingredients,
        name: 'Capuccino',
        price: 2,
